...
def solo_decision_instructions(system_platform: str = "当前系统") -> list[str]:
    return [
        ...
        (
            ...
            "严禁提前结束（CRITICAL）：\n"
            "  1. wait 和 screenshot 是辅助动作，执行后屏幕内容通常不变\n"
            "  2. 刚执行完 wait/screenshot 时，如果没有在截图中看到用户要的具体结果，禁止 finish\n"
            "  3. 任务需要提取信息（天气、新闻、价格等）时，必须先把信息写入 findings 再 finish\n"
            "  4. 你在上一步的 agent_message 中说要做什么 → 必须先做到再 finish\n"
            "  5. 如果上一步说「准备切换到7天预报」但还没切 → 继续执行，禁止 finish\n\n"
            "finish 前自检清单：\n"
            "  - 用户要的答案我看到了吗？\n"
            "  - findings 里有没有我要汇报的内容？\n"
            "  - 我上一步承诺的操作完成了吗？\n"
            "  任意一条不满足 → 继续执行，不要 finish\n\n"
