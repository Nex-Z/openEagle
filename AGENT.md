...
- **桌面执行 worker**：内部仍沿用 `solo` 协议与代码标识，通过 VL（Vision-Language）模型分析截图并操作桌面。
...
### 桌面执行流程（内部 `solo` Agent Loop）

```
用户输入任务 → client:send_message / IM 文本
  → main agent 判断需要桌面视觉操作 → route=start_solo
...
**核心设计原则**：桌面执行 worker 是一个 goal-driven agent，不是 plan executor。VL 模型在每一步都自主推理：观察屏幕、提取信息、判断进度、决定下一步。没有预定义计划，agent 完全自主决策。
...
- 完成后不再发送额外的“桌面执行已完成”状态消息，避免重复刷屏
...
**安全三层评估**：每个桌面动作经过 `assess_solo_action` 评估：safe 直接执行、confirm 等待用户确认、blocked 拒绝执行。
...
三层风险评估：`safe` / `confirm` / `blocked`，覆盖桌面执行动作、工具调用、命令执行。详见 `backend/app/safety.py`。
...
- 桌面执行 decision prompt 要求模型输出结构化 JSON，字段定义在 prompt 中说明
...
components/
  chat/           # 对话工作区（消息列表 + 输入框）
  inspector/      # 右侧执行面板（桌面执行状态、traces、assets）
